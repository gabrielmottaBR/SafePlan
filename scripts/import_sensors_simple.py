    ...
    try:
        ...
        for idx, sensor_info in enumerate(mapping['sensors'], 1):
            try:
                ...
                platform = sensor_info.get('platform')
                sensor_type = sensor_info.get('sensor_type')
                
                # Novos campos do PI AF
                sensor_id_af = sensor_info.get('id', sensor_info.get('name', f"SENSOR_{idx}"))
                descricao = sensor_info.get('descricao', sensor_info.get('name', ''))
                fabricante = sensor_info.get('fabricante', 'N/A')
                tipo_gas = sensor_info.get('tipo_gas', sensor_type)
                tipo_leitura = sensor_info.get('tipo_leitura', 'N/A')
                grupo = sensor_info.get('grupo') or 'N/A'
                uep = sensor_info.get('uep', platform)
                valor_ma = sensor_info.get('valor_ma')
                valor_pct = sensor_info.get('valor_pct')
                path_af = sensor_info.get('path', '')

                # Nome interno (único)
                internal_name = f"{platform}_{tipo_gas}_{sensor_id_af}"[:100]

                # Display name
                display_name = f"{descricao} ({grupo}/{platform})" if descricao else f"{sensor_id_af} ({platform})"
                display_name = display_name[:100]

                # Obtém thresholds padrão baseado no tipo_gas ou sensor_type
                thresholds = DEFAULT_THRESHOLDS.get(sensor_type) or DEFAULT_THRESHOLDS.get(tipo_gas.upper()) or DEFAULT_THRESHOLDS['CH4_POINT']
                unit = thresholds.get('unit', tipo_leitura if tipo_leitura != 'N/A' else 'N/A')

                # Cria sensor com TODOS os novos campos
                sensor = sensor_mgr.create_sensor(
                    ...
                    platform=platform,
                    unit=unit,
                    pi_server_tag=path_af or descricao,
                ...
                if sensor:
                    print(f"✓ [{idx}/{len(mapping['sensors'])}] {internal_name} ({tipo_gas}/{unit})")
                    imported_count += 1
